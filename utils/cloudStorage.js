...
const filterEntryForSync = async (entry) => {
  ...
  if (syncSettings.media) {
    if (entry.images !== undefined) filteredEntry.images = entry.images;
    if (entry.audio !== undefined) filteredEntry.audio = entry.audio;
  }
...
export const getEntriesFromCloud = async () => {
  ...
  try {
    ...
    console.log(`✅ Fetched ${entries.length} entries from cloud`);
    return entries;
  } catch (error) {
...
export const syncLocalToCloud = async () => {
  ...
  try {
    ...
    console.log(`✅ Synced ${syncedCount} entries to cloud`);
...
export const subscribeToCloudChanges = (callback) => {
  ...
  const unsubscribe = onSnapshot(q, (snapshot) => {
    ...
    if (changes.length > 0) {
      console.log(`🔄 Real-time changes: ${changes.length}`);
      callback(changes);
    }
  });
