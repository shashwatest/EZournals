...
const persistMoodTags = async (tags) => {
  ...
  await PlatformStorage.setItem(getStorageKey(), JSON.stringify(normalized));
  if (auth.currentUser) {
...
export const getMoodTags = async () => {
  try {
    const stored = await PlatformStorage.getItem(getStorageKey());
    if (stored) {
...
export const addMoodTag = async (name, customColor = null) => {
  ...
  const nextTag = {
    name: trimmedName,
    color: customColor || FALLBACK_COLORS[existing.length % FALLBACK_COLORS.length],
  };
